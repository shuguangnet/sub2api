      ...
      <Transition name="select-dropdown">
        ...
        >
          ...
          <div v-if="isSearchable" class="select-search">
            <Icon name="search" size="sm" class="text-ink-subtle" />
            <input
              ref="searchInputRef"
              v-model="searchQuery"
              type="text"
            ...
            >
              <slot name="option" :option="option" :selected="isSelected(option)">
                <Icon
                  ...
                  name="search"
                  size="sm"
                  class="flex-shrink-0 text-ink-subtle"
                />
                <span class="select-option-label" :class="option._creatable && 'italic text-ink-muted'">{{ getOptionLabel(option) }}</span>
                <Icon
...
.select-icon {
  @apply flex-shrink-0 text-ink-subtle;
}

.select-clear {
  @apply flex flex-shrink-0 cursor-pointer items-center justify-center;
  @apply rounded text-ink-subtle transition-colors;
  @apply hover:text-ink-muted;
}
</style>
...
.select-dropdown-portal .select-search-input {
  ...
  @apply text-ink-strong;
  @apply placeholder:text-ink-subtle;
