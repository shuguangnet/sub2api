        ...
        <div class="flex justify-end gap-3">
          ...
          <div class="relative" ref="columnDropdownRef">
            ...
            <div
              ...
              class="absolute right-0 top-full z-50 mt-1 max-h-80 w-48 overflow-y-auto rounded-lg border border-line bg-surface-elevated py-1 shadow-popover"
            >
              <button
                v-for="col in toggleableColumns"
                :key="col.key"
                @click="toggleColumn(col.key)"
                class="flex w-full items-center justify-between px-4 py-2 text-left text-sm text-ink hover:bg-surface-2"
              >
                <span>{{ col.label }}</span>
                <Icon
        ...
        >
          <template #cell-key="{ value, row }">
            <div class="flex items-center gap-2">
              ...
              <button
                ...
                class="rounded-lg p-1 transition-colors hover:bg-surface-2"
                :class="
                  copiedKeyId === row.id
                    ? 'text-green-500'
          ...
          <template #cell-name="{ value, row }">
            <div class="flex items-center gap-1.5">
              <span class="font-medium text-ink-strong">{{ value }}</span>
              <Icon
          ...
          <template #cell-group="{ row }">
            <div class="group/dropdown relative">
              <button
                ...
                class="-mx-2 -my-1 flex cursor-pointer items-center gap-2 rounded-lg px-2 py-1 transition-all duration-200 hover:bg-surface-2"
              ...
              >
                ...
                />
<span v-else class="text-sm text-ink-subtle">{{
                  t('keys.noGroup')
                }}</span>
                <span class="text-xs text-ink-muted">{{ t('keys.selectGroup') }}</span>
                <svg
                  class="h-3.5 w-3.5 text-ink-subtle opacity-60 transition-opacity group-hover/dropdown:opacity-100"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                  stroke-width="2"
          ...
          <template #cell-usage="{ row }">
            <div class="text-sm">
              <div class="flex items-center gap-1.5">
                <span class="text-ink-muted">{{ t('keys.today') }}:</span>
                <span class="font-medium text-ink-strong">
                  ${{ (usageStats[row.id]?.today_actual_cost ?? 0).toFixed(4) }}
                </span>
              </div>
              <div class="mt-0.5 flex items-center gap-1.5">
                <span class="text-ink-muted">{{ t('keys.total') }}:</span>
                <span class="font-medium text-ink-strong">
              ...
              <div v-if="row.quota > 0" class="mt-1.5">
                <div class="flex items-center gap-1.5">
                  <span class="text-ink-muted">{{ t('keys.quota') }}:</span>
                  <span :class="[
                    'font-medium',
            ...
            <div v-if="row.rate_limit_5h > 0 || row.rate_limit_1d > 0 || row.rate_limit_7d > 0" class="space-y-1.5 min-w-[140px]">
              ...
              <div v-if="row.rate_limit_5h > 0">
                <div class="flex items-center justify-between text-xs">
                  <span class="text-ink-muted">5h</span>
                  <span :class="[
                ...
                <div v-if="row.reset_5h_at && formatResetTime(row.reset_5h_at)" class="text-[10px] text-ink-subtle tabular-nums">
              ...
              <div v-if="row.rate_limit_1d > 0">
                <div class="flex items-center justify-between text-xs">
                  <span class="text-ink-muted">1d</span>
                  <span :class="[
                    'font-medium tabular-nums',
                ...
                <div v-if="row.reset_1d_at && formatResetTime(row.reset_1d_at)" class="text-[10px] text-ink-subtle tabular-nums">
              ...
              <div v-if="row.rate_limit_7d > 0">
                <div class="flex items-center justify-between text-xs">
                  <span class="text-ink-muted">7d</span>
                  <span :class="[
                    'font-medium tabular-nums',
                ...
                <div v-if="row.reset_7d_at && formatResetTime(row.reset_7d_at)" class="text-[10px] text-ink-subtle tabular-nums">
              ...
              <button
                ...
                class="mt-0.5 inline-flex items-center gap-1 rounded px-1.5 py-0.5 text-xs text-ink-muted transition-colors hover:bg-surface-2 hover:text-primary-600 dark:hover:text-primary-400"
          ...
          <template #cell-expires_at="{ value }">
            <span v-if="value" :class="[
              'text-sm',
              new Date(value) < new Date() ? 'text-red-500 dark:text-red-400' : 'text-ink-muted'
            ]">
              {{ formatDateTime(value) }}
            </span>
            <span v-else class="text-sm text-ink-subtle">{{ t('keys.noExpiration') }}</span>
          </template>
          ...
          <template #cell-last_used_at="{ value }">
            <span v-if="value" class="text-sm text-ink-muted">
              {{ formatDateTime(value) }}
            </span>
            <span v-else class="text-sm text-ink-subtle">-</span>
          </template>
          ...
          <template #cell-created_at="{ value }">
            <span class="text-sm text-ink-muted">{{ formatDateTime(value) }}</span>
          </template>
            ...
            <div class="flex items-center gap-1">
              ...
              <button
                ...
                :class="[
                  ...
                  row.status === 'active'
                    ? 'text-ink-muted hover:bg-yellow-50 hover:text-yellow-600 dark:hover:bg-yellow-900/20 dark:hover:text-yellow-400'
                    : 'text-ink-muted hover:bg-green-50 hover:text-green-600 dark:hover:bg-green-900/20 dark:hover:text-green-400'
                ]"
              >
              ...
              <button
                @click="editKey(row)"
                class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-ink-muted transition-colors hover:bg-surface-2 hover:text-primary-600 dark:hover:text-primary-400"
              >
        ...
        <div>
          ...
          >
            <template #selected="{ option }">
              ...
              />
              <span v-else class="text-ink-subtle">{{ t('keys.selectGroup') }}</span>
            </template>
          ...
          <div class="flex items-center justify-between">
            ...
            <button
              ...
              :class="[
                ...
                formData.use_custom_key ? 'bg-primary-500' : 'bg-surface-3'
              ]"
            >
              <span
                :class="[
        ...
        <div class="space-y-3">
          <div class="flex items-center justify-between">
            ...
            <button
              ...
              :class="[
                'relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none',
                formData.enable_ip_restriction ? 'bg-primary-500' : 'bg-surface-3'
              ]"
            >
              <span
                :class="[
        ...
        <div class="space-y-3">
          ...
          <div class="flex items-center justify-between">
            ...
            <button
              ...
              :class="[
                'relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none',
                formData.enable_quota ? 'bg-primary-500' : 'bg-surface-3'
              ]"
            >
              <span
                :class="[
        ...
        <div class="space-y-3">
          <div class="flex items-center justify-between">
            ...
            <button
              ...
              :class="[
                'relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none',
                formData.enable_rate_limit ? 'bg-primary-500' : 'bg-surface-3'
              ]"
            >
              <span
                :class="[
          ...
          <div v-if="formData.enable_rate_limit" class="space-y-4 pt-2">
            ...
            <div>
              ...
              <div v-if="showEditModal && selectedKey && selectedKey.rate_limit_5h > 0" class="mt-2">
                <div class="flex items-center gap-2">
                  <div class="flex-1 rounded-lg bg-surface-2 px-3 py-2 text-sm">
                    <span :class="[
                      'font-medium',
        ...
        <div class="space-y-3">
          <div class="flex items-center justify-between">
            ...
            <button
              ...
              :class="[
                'relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none',
                formData.enable_expiration ? 'bg-primary-500' : 'bg-surface-3'
              ]"
            >
              <span
                :class="[
          ...
          <div v-if="formData.enable_expiration" class="space-y-4 pt-2">
            ...
            <div v-if="showEditModal && selectedKey?.expires_at" class="text-sm">
              <span class="text-ink-muted">{{ t('keys.currentExpiration') }}: </span>
              <span class="font-medium text-ink-strong">
          ...
          {{ t('keys.ccsClientSelect.description') }}
	        ...
	        <div class="grid grid-cols-2 gap-3">
	          <button
	            ...
	            class="flex flex-col items-center gap-2 p-4 rounded-xl border-2 border-line hover:border-primary-500 dark:hover:border-primary-500 hover:bg-primary-50 dark:hover:bg-primary-500/10 transition-all"
	          >
	            <Icon name="terminal" size="xl" class="text-ink-muted" />
	            <span class="font-medium text-ink-strong">{{
	          ...
	          <button
	            ...
	            class="flex flex-col items-center gap-2 p-4 rounded-xl border-2 border-line hover:border-primary-500 dark:hover:border-primary-500 hover:bg-primary-50 dark:hover:bg-primary-500/10 transition-all"
	          >
	            <Icon name="sparkles" size="xl" class="text-ink-muted" />
	            <span class="font-medium text-ink-strong">{{
    ...
    <Teleport to="body">
      <div
        ...
        ref="dropdownRef"
        class="animate-in fade-in slide-in-from-top-2 fixed z-[100000020] w-max min-w-[380px] overflow-hidden rounded-xl bg-surface-elevated shadow-popover ring-1 ring-black/5 duration-200 dark:ring-white/10"
      ...
      >
        ...
        <div class="border-b border-line p-2">
          <div class="relative">
            <svg class="absolute left-2.5 top-1/2 h-4 w-4 -translate-y-1/2 text-ink-subtle" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
              <path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
            </svg>
            <input
              v-model="groupSearchQuery"
              type="text"
              class="w-full rounded-lg border border-line bg-surface-2 py-1.5 pl-8 pr-3 text-sm text-ink-strong placeholder-ink-subtle outline-none focus:border-primary-300 focus:ring-1 focus:ring-primary-300 dark:focus:border-primary-600 dark:focus:ring-primary-600"
        ...
        <div class="max-h-80 overflow-y-auto p-1.5">
          ...
          <div v-if="filteredGroupOptions.length === 0" class="py-4 text-center text-sm text-ink-subtle">
            {{ t('keys.noGroupFound') }}
          </div>
        </div>
      </div>
